"""SACRIFICIAL_PREFLIGHT_FUSION_V9_FEASIBILITY_V1 — frozen writer prompt comparison.
...
the ONLY delta is the Writer prompt version):
  ...
  F9: Writer v8 ("Sacrificial Preflight Fusion v9", xml_story) — fusion candidate.
...
Discipline (pre-registered):
  ...
  - Call order (P7->F9 or F9->P7) stratified-randomized per replica and recorded.
...
EXPERIMENT = "SACRIFICIAL_PREFLIGHT_FUSION_V9_FEASIBILITY_V1"
SEED = 202607208
REPLICAS = 3
GROUPS = ("P7", "F9")
...
def check_v9_structure(draft_notes: str) -> dict[str, Any]:
    """Check F9 draft_notes for the eight required sections.
...
def process_writer_output(
    ...
    # Add F9 structural checks
    if group == "F9" and draft_notes:
        meta["v9_structure"] = check_v9_structure(draft_notes)
...
def writer_override_for_group(
    ...
    Both P7 and F9 use identical parameters; only prompt_version_id differs."""
    return {
        "provider_id": FROZEN["provider_id"],
        "model_id": FROZEN["model_id"],
        "prompt_version_id": prompt_version_id,
...
def slot_assertions(
    ...
    checks = {
        ...
        "group_F9_texts": len([q for q in queued if q[1]["group"] == "F9"])
        == len(case_ids) * REPLICAS,
        "per_case_6": all(
...
def dry_run_report(root: Path, case_ids: list[str]) -> str:
    ...
    lines = [
        ...
        "  F9: Writer v9 'Sacrificial Preflight Fusion v9' (xml_story)",
        "",
...
async def run_case(
    ...
    f9_version_number: int,
) -> dict[str, Any]:
    """One scene: 6 Writers (P7 x 3 + F9 x 3, sequential within scene).
        ...
        for replica in range(1, REPLICAS + 1):
            ...
            for group in order:
                ...
                if group == "P7":
                    ...
                    prompt_version_id = f9_version_id
                    prompt_name = f9_prompt_name
                    prompt_profile_id = f9_profile_id
                    prompt_version_number = f9_version_number
...
async def run(root: Path, database: Path, dry_run: bool) -> None:
    ...
    # Ensure P7 and F9 prompts exist in all isolated DBs
    p7_ids: dict[str, str] = {}
    f9_ids: dict[str, str] = {}
    p7_profile_ids: dict[str, str] = {}
    f9_profile_ids: dict[str, str] = {}
    ...
    for case_id in case_ids:
        async with factories[case_id]() as session:
            ...
            f9_id = await ensure_prompt_exists(
                session, "Sacrificial Preflight Fusion v9"
            )
            await session.commit()
    ...
    async with factories[first_case]() as session:
        manifest: dict[str, Any] = {
            ...
            "f9_prompt_version_id": f9_version_id,
            "p7_profile_id": p7_profile_id,
            "f9_profile_id": f9_profile_id,
            ...
            "rules": [
                ...
                "2 groups x 3 replicas per scene; stratified-random P7/F9 call order per replica.",
                ...
                "The ONLY delta between P7 and F9 is the Writer prompt version.",
            ],
            "success_criteria": {
                "format_and_mechanism": {
                    "A1": "F9 at least 11/12 successfully extract story",
                    "A2": "F9 successful outputs: XML leak = 0",
                    "A3": "F9 successful outputs: draft_notes leak = 0",
                    "A4": "F9 at least 10/12 contain 8-section draft_notes structure",
                    "A5": "F9 at least 9/12 contain 6-9 narrative units in section 7",
                },
                "length": {
                    "B1": "F9 at least 10/12 reach 2000 chars",
                    "B2": "F9 story median >= 2200 chars",
                    "B3": "F9 mean story chars >= P7 mean by 25%",
                    "B4": "In pairs where P7 < 2000, F9 crosses 2000 in at least 2/3",
                    "B5": "F9 at least 8/12 reach 2000 chars (anti-mean-skew floor)",
                },
                "quality": {
                    "C1": "'Better as final text': F9 wins >= 7/12",
                    "C2": "'More complete chapter': F9 wins >= 7/12",
                    "C3": "F9 padded_repetition tags <= 3",
                    "C4": "F9 delayed_stop tags <= 3",
                    "C5": "F9 early_stop tags <= 2",
                    "C6": "F9 missing_context or process_jump tags <= 2",
                    "C7": "F9 does not lose 0:3 in any single scene",
                },
                "zhuque": {
                    "D1": "F9 char-weighted human rate not < P7 by >5pp",
                    "D2": "F9 per-article human rate median not < P7 by >5pp",
                    "D3": "F9 wins median in >= 2/4 scenes",
                    "D4": "F9 human-rate >= 60% count not < P7 by >= 2",
                },
                "overall_judgment": {
        ...
        f9_identity = await prompt_identity(session, f9_version_id)
        manifest["p7_prompt_identity"] = p7_identity
        manifest["f9_prompt_identity"] = f9_identity
    ...
    assert f9_identity["output_mode"] == "xml_story", (
        f"F9 output_mode must be xml_story, got {f9_identity['output_mode']}"
    )
    assert p7_version_id != f9_version_id, (
        "P7 and F9 must be different prompt versions"
    )
    assert p7_identity["system_template_sha256"] != f9_identity["system_template_sha256"], (
        "P7 and F9 system templates must differ"
    )
    assert p7_identity["user_template_sha256"] != f9_identity["user_template_sha256"], (
        "P7 and F9 user templates must differ"
    )
    assert p7_profile_id != f9_profile_id, (
        "P7 and F9 must be different prompt profiles"
    )
    ...
    print(f"  F9.name={f9_prompt_name}")
    print(f"  P7.output_mode={p7_identity['output_mode']}")
    print(f"  F9.output_mode={f9_identity['output_mode']}")
    print(f"  P7.id={p7_version_id}")
    print(f"  F9.id={f9_version_id}")
    print(f"  P7 != F9 -> {p7_version_id != f9_version_id}")
        ...
        exported: list[dict[str, Any]] = await asyncio.gather(
            *(
                run_case(
                    ...
                    f9_ids.get(case[0], f9_version_id),
                    p7_profile_ids.get(case[0], p7_profile_id),
                    f9_profile_ids.get(case[0], f9_profile_id),
                    p7_prompt_name,
                    f9_prompt_name,
                    p7_identity.get("version_number", 1),
                    f9_identity.get("version_number", 1),
                )
                for case in SCENES
            )
        )
    ...
    f9_drafts = [d for d in all_drafts if d.get("group") == "F9"]
    ...
    # Post-run assertions
    _run_post_hoc_assertions(all_drafts, p7_drafts, f9_drafts)
    ...
    # Summary
    _print_statistics(all_drafts, p7_drafts, f9_drafts, wall_seconds)
        ...
        "drafts": [
            {
                ...
                "v9_structure": d.get("metadata", {}).get("v9_structure")
                if d.get("group") == "F9" else None,
            }
...
def _run_post_hoc_assertions(
    ...
    print(f"  F9 slots: {f9_ok} (expected {len(case_ids) * REPLICAS})")
    print(f"  Per scene == 6: {all(len([d for d in all_drafts if d.get('case_id') == cid]) == 6 for cid in case_ids)}")
    print(f"  Per scene per group == 3: {all(len([d for d in p7_drafts if d.get('case_id') == cid]) == 3 for cid in case_ids) and all(len([d for d in f9_drafts if d.get('case_id') == cid]) == 3 for cid in case_ids)}")
...
def _print_statistics(
    ...
    """Print summary statistics for P7 vs F9."""
    print(f"\n{'='*60}")
    print(f"  EXPERIMENT RESULTS")
    print(f"{'='*60}")
    print(f"  Total wall time: {wall_seconds}s")

    for label, drafts in [("P7 (v7)", p7_drafts), ("F9 (v9)", f9_drafts)]:
        def _meta(d, key, default=0):
            m = d.get("metadata", {}) or {}
            return m.get(key, d.get(key, default))
        ...
        if label == "F9 (v9)":
            v9_structures = [
                d.get("metadata", {}).get("v9_structure", {})
                for d in drafts
                if d.get("metadata", {}).get("v9_structure")
            ]
            if v9_structures:
                eight_sections = sum(
                    1 for v in v9_structures if v.get("has_eight_sections")
                )
                unit_6_9 = sum(
                    1 for v in v9_structures if v.get("has_unit_count_6_9")
                )
                target_min = sum(
                    1 for v in v9_structures if v.get("has_target_length_minimum")
                )
                char_estimates = sum(
                    1 for v in v9_structures if v.get("has_estimated_char_counts")
                )
                print(f"  Eight sections complete: {eight_sections}/{len(v9_structures)}")
                print(f"  6-9 narrative units: {unit_6_9}/{len(v9_structures)}")
                print(f"  Target length as minimum: {target_min}/{len(v9_structures)}")
                print(f"  Char estimates: {char_estimates}/{len(v9_structures)}")
    ...
    for key in sorted(set(list(p7_by_key.keys()) + list(f9_by_key.keys()))):
        if key not in p7_by_key or key not in f9_by_key:
            continue
        p7 = p7_by_key[key]
        f8 = f9_by_key[key]
        p7_len = _story_len(p7)
        f9_len = _story_len(f8)
        pairs_compared += 1
        diff = f9_len - p7_len
        print(f"    {key[0]} R{key[1]}: P7={p7_len} F9={f9_len} diff={diff:+d}")
        if f9_len > p7_len:
            f9_longer += 1
        if p7_len > 0 and p7_len < 2000 and f9_len >= 2000:
            p7_under_2000_f8_over += 1
        if p7_len > 0 and p7_len < 2000:
            p7_under_2000_pairs += 1

    if pairs_compared > 0:
        print(f"    F9 longer in: {f9_longer}/{pairs_compared}")
        print(f"    P7<2000 & F9>=2000: {p7_under_2000_f8_over}/{p7_under_2000_pairs}")
...
def make_blind_assets(
    ...
    (anonymized same-scene same-replica P7/F9 pairs, X/Y randomized)."""
    pieces: dict[str, Any] = {}
    pairs: dict[str, Any] = {}
    ...
    for case in exported:
        ...
        for replica in range(1, REPLICAS + 1):
            ...
            f9 = by_key.get(("F9", replica))
            if (
                not p7 or not f9
                or not p7.get("story_path")
                or not f9.get("story_path")
            ):
                continue
            ...
            y_group = "F9" if x_group == "P7" else "P7"
            pair_cards.append({
                "pair_id": pair_id,
...
def package_zhuque_submission(
    ...
    write_json(
        zhuque_dir / "zhuque_submission_manifest.json",
        {
            ...
            "f9_count": sum(1 for c in cards if c["group"] == "F9"),
    ...
    print(
        ...
        f"F9={sum(1 for c in cards if c['group']=='F9')} "
        f"— all tests pass"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=REPO_ROOT
        / "__evaluation"
        / "sacrificial_preflight_fusion_v9_feasibility_v1",
    )
    parser.add_argument(
        "--database",
        type=Path,
        default=REPO_ROOT
        / "__evaluation"
        / "sacrificial_preflight_fusion_v9_feasibility_v1.sqlite3",
    )
