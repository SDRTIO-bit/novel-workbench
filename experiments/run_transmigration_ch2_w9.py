"""Retry CH2 with W9 (less strict tempo)."""
...
WRITER = {"name": "Sacrificial Preflight Fusion v9", "mode": "writer_brief"}  # W9, less strict
...
async def run_chapter(db_path):
    ...
    async with factory() as session:
        ...
        proj_name = "transmigration_CH2_w9"
...
async def run():
    ...
    if result and result["story"] and not result["error"]:
        path = EVIDENCE / "CH2_w9.txt"
        path.write_text(result["story"], encoding="utf-8")
        print(f"\nCH2 W9 retry SUCCESS: {path}")
        # Reassemble full 3-chapter file
        ...
        print("Updated combined file with all 3 chapters.")
    else:
        print("\nCH2 W9 retry FAILED.")
