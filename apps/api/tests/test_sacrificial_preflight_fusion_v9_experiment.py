...
def _runner_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "run_sacrificial_preflight_fusion_v9_feasibility_v1",
        root
        / "experiments"
        / "run_sacrificial_preflight_fusion_v9_feasibility_v1.py",
    )
...
def test_f9_name(runner):
    entry = runner._find_builtin_prompt("Sacrificial Preflight Fusion v9")
    assert entry is not None
    assert entry["stage"] == "writer"
    assert entry["output_mode"] == "xml_story"


def test_p7_f9_different_entries(runner):
    p7 = runner._find_builtin_prompt("Sacrificial Preflight v7")
    f9 = runner._find_builtin_prompt("Sacrificial Preflight Fusion v9")
    assert p7 is not None
    assert f9 is not None
    assert p7["name"] != f9["name"]
    assert p7["system_template"] != f9["system_template"]
    assert p7["user_template"] != f9["user_template"]


# ── Prompt hashes ──────────────────────────────────────────────────────


def test_p7_f9_prompt_hash_different(runner):
    p7 = runner._find_builtin_prompt("Sacrificial Preflight v7")
    f9 = runner._find_builtin_prompt("Sacrificial Preflight Fusion v9")
    p7_sys_hash = hashlib.sha256(
        p7["system_template"].encode("utf-8")
    ).hexdigest()
    f9_sys_hash = hashlib.sha256(
        f9["system_template"].encode("utf-8")
    ).hexdigest()
    p7_usr_hash = hashlib.sha256(
        p7["user_template"].encode("utf-8")
    ).hexdigest()
    f9_usr_hash = hashlib.sha256(
        f9["user_template"].encode("utf-8")
    ).hexdigest()
    assert p7_sys_hash != f9_sys_hash
    assert p7_usr_hash != f9_usr_hash
...
def test_writer_override_factory(runner):
    ...
    f9 = runner.writer_override_for_group(
        "F9", "test-f9-id", "Sacrificial Preflight Fusion v9"
    )
    for override in (p7, f9):
    ...
    for key in frozen_keys:
        assert p7[key] == f9[key], key
    # Prompt version must differ
    assert p7["prompt_version_id"] != f9["prompt_version_id"]
    assert p7["_policy_metadata"]["group"] == "P7"
    assert f9["_policy_metadata"]["group"] == "F9"
...
def test_writer_override_no_extra_fields(runner):
    ...
    assert "_instruction_block" not in f9
    assert "writer_behavior_mode" not in p7
    assert "writer_behavior_mode" not in f9
...
def test_blind_token_deterministic_and_group_sensitive(runner):
    ...
    assert len(token) == 12
    assert token != runner.make_blind_token("CO-04", "F9", 1)
...
def test_v9_structure_all_sections(runner):
...
def test_v9_unit_count_6_9_detected(runner):
...
def test_v9_target_length_minimum(runner):
...
def test_v9_unit_char_estimates(runner):
...
def test_dry_run_asserts_prompt_identity(runner):
    ...
    assert "Sacrificial Preflight Fusion v9" in report
...
def test_zhuque_no_xml_tags_in_submission(runner, tmp_path):
    ...
    assert len(boundaries) == 1  # Only P7-1 has text, F9 had 0 chars
...
def test_v9_checks_do_not_modify_status(runner):
