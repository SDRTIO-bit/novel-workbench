"""Integration tests for the planner v2 contract boundary.

The planner v2 contract is enforced only when the resolved prompt version
carries ``output_schema_name == "planner_v2"`` — a deterministic marker
stored on the prompt version row.  Custom prompts, older built-in versions,
and user-edited variants of the built-in profile all carry a different or
empty schema name and retain v1 compatibility.
"""
import json
...
def _planner_v1_payload() -> str:
    return json.dumps({
        ...
        "characters": [], "pressure": "即将交班",
        "turning_point": "敲击声", "end_condition": "切断电源", "forbidden": [],
...
def _planner_v2_payload() -> str:
    return json.dumps({
        ...
        "characters": [], "pressure": "即将交班",
        "turning_point": "敲击声再响", "end_condition": "切断电源", "forbidden": [],
        "causal_transitions": [{
            "id": "CT01", "kind": "evidence_to_action",
            "visible_trigger": "冷却管传出敲击声",
            "character_interpretation": "林隅认为来自活物",
            "character_next_action": "林隅切断外门电源",
            "rejected_alternative": "按原流程上报",
            "immediate_consequence": "仓库完全断电隔离",
            "counterfactual_without_action": "不切断会引来保安",
            "consequence_would_still_happen": False,
            "state_delta": {"before": "仓库正常供电", "after": "仓库完全断电"},
            "cost_or_commitment": "承担擅自断电的责任",
            "next_constraint": "必须在备用电源耗尽前查明来源",
            "reader_must_infer": "敲击声不是普通故障",
            "narrator_must_not_state": ["敲击声真实来源"],
        }],
...
async def _make_profile(db_session, stage, *, builtin, name, marks, schema_names):
    profile = PromptProfile(stage=stage, name=name, is_builtin=builtin)
    db_session.add(profile)
    await db_session.flush()
    versions = []
    for i, (mark, schema) in enumerate(zip(marks, schema_names), start=1):
...
async def _make_workflow(db_session, prompt_version_id):
...
def test_expected_version_only_for_planner_v2_schema():
    assert _expected_planner_contract_version("planner", {"output_schema_name": PLANNER_V2_SCHEMA_NAME}) == 2
    assert _expected_planner_contract_version("planner", {"output_schema_name": "planner"}) is None
    assert _expected_planner_contract_version("planner", {"output_schema_name": ""}) is None
    assert _expected_planner_contract_version("planner", None) is None
    assert _expected_planner_contract_version("critic", {"output_schema_name": PLANNER_V2_SCHEMA_NAME}) is None


# ── prompt resolution metadata ────────────────────────────────────────


async def test_resolve_prompt_exposes_output_schema_name(db_session):
    _, versions = await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["old", "new"],
        schema_names=["planner", "planner_v2"],
    )
    ...
    assert meta_old["output_schema_name"] == "planner"
    assert meta_new["output_schema_name"] == "planner_v2"


async def test_resolve_prompt_custom_profile_is_not_v2(db_session):
    _, versions = await _make_profile(
        db_session, "planner", builtin=False, name="自定义规划",
        marks=["custom"], schema_names=["planner"],
    )
    svc = ContextService(db_session)
    _, _, meta = await svc._resolve_prompt("planner", None, versions[0].id)
    assert meta["output_schema_name"] == "planner"
...
async def test_resolve_prompt_via_workflow_step(db_session):
    _, versions = await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["v1", "v2"],
        schema_names=["planner", PLANNER_V2_SCHEMA_NAME],
    )
    ...
    assert meta_old["output_schema_name"] == "planner"
    assert meta_new["output_schema_name"] == PLANNER_V2_SCHEMA_NAME


async def test_resolve_prompt_builtin_fallback_respects_schema(db_session):
    await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["only"], schema_names=[PLANNER_V2_SCHEMA_NAME],
    )
    svc = ContextService(db_session)
    _, _, meta = await svc._resolve_prompt("planner", None, None)
    assert meta["output_schema_name"] == PLANNER_V2_SCHEMA_NAME
...
async def _run_planner(db_session, monkeypatch, *, prompt_version_id, provider_text):
    ...
    run = await svc.repo.create_run(
        project_id=project.id, chapter_id=None,
        workflow_profile_id=workflow.id, scene_instruction="写第一章",
    )
    return await svc.execute_stage(run.id, "planner", {})


async def test_builtin_planner_v2_rejects_v1_output(db_session, monkeypatch):
    _, versions = await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["v2"], schema_names=[PLANNER_V2_SCHEMA_NAME],
    )
...
async def test_builtin_planner_v2_accepts_valid_v2_output(db_session, monkeypatch):
    _, versions = await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["v2"], schema_names=[PLANNER_V2_SCHEMA_NAME],
    )
...
async def test_custom_prompt_keeps_v1_compatibility(db_session, monkeypatch):
    _, versions = await _make_profile(
        db_session, "planner", builtin=False, name="自定义规划",
        marks=["custom"], schema_names=[""],
    )
    candidate = await _run_planner(
        db_session, monkeypatch,
        prompt_version_id=versions[-1].id,
...
async def test_old_builtin_version_without_v2_schema_is_legacy(db_session, monkeypatch):
    _, versions = await _make_profile(
        db_session, "planner", builtin=True, name="默认场景规划",
        marks=["old", "new"],
        schema_names=["planner", PLANNER_V2_SCHEMA_NAME],
